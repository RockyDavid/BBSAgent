  ...
  for (var _ = -1, n = len; ++_ < n; ) {
    ...
    if (Ansi.state) {
      ...
      switch (Ansi.str.slice(-1)) {
        case "m":
          ScreenRow[g_cursor.row] = addAnsiAttrSeq(
            ScreenRow[g_cursor.row],
            g_cursor.col,
            Ansi.str,
          );
          Ansi.state = false;
        ...
        case "K":
          if (g_cursor.row != 25) {
            /* fix for some article EX: document/CollectingArticle53617 */
            ScreenRow[g_cursor.row] = addAnsiEOLSeq(
              ScreenRow[g_cursor.row],
              g_cursor.col,
              Ansi.str,
            );
        ...
        case "J":
          ...
          if (S(Ansi.str).contains("[2J")) {
            ...
            console.log(
              "Warning: parseNewdata() got undefined ANSI-pattern for J",
            );
          }
          Ansi.state = false;
          Ansi.str = "no-ansi";
        ...
        default:
          //do nothing
          if (Ansi.str.length > 25)
            console.log("Warning: parseNewdata() got undefined ANSI-pattern"); //alert if no ANSI-pattern matches.
      }
    } else {
      //in non-ANSI state
      switch (ch) {
        ...
        default: //normal character
          ScreenRow[g_cursor.row] = replaceCharAt(
            ScreenRow[g_cursor.row],
            g_cursor.col,
            ch,
          );
...
function eraseOldAnsi(OldAnsi, str, wordCursor) {
  ...
  **/
  ...
  return (
    str.substr(0, preWordIndex + 1) +
    str.substr(preWordIndex + OldAnsi.length + 1)
  );
}
